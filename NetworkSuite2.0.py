...
class SubnetCalculatorTab:
    ...
    def create_widgets(self):
        ...
        # Inputs
        ttk.Label(main_frame, text="IP Address:", font=('Helvetica', 10, 'bold')).grid(
        ...
        ttk.Label(main_frame, text="Subnet Mask / CIDR:", font=('Helvetica', 10, 'bold')).grid(
            row=2, column=0, sticky=tk.W, pady=5)
        self.mask_entry = ttk.Entry(main_frame, width=30)
        ...
        # Calculate Button (Success color)
        calc_button = ttk.Button(main_frame, text="Calculate", command=self.calculate, bootstyle=SUCCESS, width=20)
        calc_button.grid(row=4, column=0, columnspan=2, pady=10)
        
        results_frame = ttk.Labelframe(main_frame, text="Results", padding="15", bootstyle="info")
        ...
        for idx, (label_text, key) in enumerate(labels):
            ttk.Label(results_frame, text=label_text, 
                     font=('Helvetica', 9, 'bold')).grid(row=idx, column=0, 
                                                      sticky=tk.W, pady=3)
            self.result_labels[key] = ttk.Label(results_frame, text="—", 
                                               font=('Helvetica', 9), bootstyle="inverse-info")
            self.result_labels[key].grid(row=idx, column=1, sticky=tk.W, 
                                        padx=(10, 0), pady=3)
        
        # Clear Button (Outline Danger)
        clear_button = ttk.Button(main_frame, text="Clear", command=self.clear_results, bootstyle="danger-outline")
    ...
    def calculate(self):
        try:
            ...
            for lbl in self.result_labels.values():
                lbl.config(text="—", bootstyle="inverse-info")
...
class NetworkScannerTab:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(header_frame, text="Network Ping Scanner", font=("Helvetica", 16, "bold"), bootstyle="primary")
        title_label.pack()
        
        subtitle_label = ttk.Label(header_frame, text="Scan any network from /0 to /32 subnet mask", font=("Helvetica", 9), bootstyle="secondary")
        subtitle_label.pack()
        
        input_frame = ttk.Labelframe(self.parent, text="Network Configuration", padding="10", bootstyle="info")
        ...
        ttk.Label(input_frame, text="(e.g., 192.168.1.0)", bootstyle="secondary").grid(row=0, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Label(input_frame, text="(/0 to /32)", bootstyle="secondary").grid(row=1, column=2, sticky=tk.W, padx=5)
        ...
        self.scan_button = ttk.Button(button_frame, text="Start Scan", command=self.start_scan, width=15, bootstyle=SUCCESS)
        self.scan_button.pack(side=tk.LEFT, padx=5)
        
        self.stop_button = ttk.Button(button_frame, text="Stop Scan", command=self.stop_scan, state=tk.DISABLED, width=15, bootstyle=DANGER)
        self.stop_button.pack(side=tk.LEFT, padx=5)
        
        self.dns_button = ttk.Button(button_frame, text="DNS Lookup", command=self.perform_dns_lookup, state=tk.DISABLED, width=15, bootstyle=INFO)
        self.dns_button.pack(side=tk.LEFT, padx=5)
        
        self.save_button = ttk.Button(button_frame, text="Save Results", command=self.save_results, state=tk.DISABLED, width=15, bootstyle=PRIMARY)
        self.save_button.pack(side=tk.LEFT, padx=5)
        
        self.clear_button = ttk.Button(button_frame, text="Clear Results", command=self.clear_results, width=15, bootstyle="secondary-outline")
        self.clear_button.pack(side=tk.LEFT, padx=5)
        
        progress_frame = ttk.Labelframe(self.parent, text="Scan Progress", padding="10", bootstyle="success")
        progress_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(progress_frame, variable=self.progress_var, maximum=100, length=300, bootstyle="striped-success")
        self.progress_bar.pack(fill=tk.X, pady=5)
        
        self.status_label = ttk.Label(progress_frame, text="Ready to scan", font=("Helvetica", 9))
        self.status_label.pack()
        
        results_frame = ttk.Labelframe(self.parent, text="Scan Results", padding="10", bootstyle="info")
        results_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        
        # Using ttkbootstrap's ScrolledText
        self.results_text = ScrolledText(results_frame, width=80, height=20, font=("Consolas", 9), autohide=True)
        self.results_text.pack(fill=tk.BOTH, expand=True)
        
        # Configure tags (Colors adapted for dark theme)
        self.results_text.tag_config("header", foreground="#5bc0de", font=("Consolas", 9, "bold"))
        self.results_text.tag_config("alive", foreground="#5cb85c") # Success Green
        self.results_text.tag_config("info", foreground="#f7f7f9") # White/Light
        self.results_text.tag_config("error", foreground="#d9534f") # Danger Red
        self.results_text.tag_config("dns", foreground="#f0ad4e") # Warning/Orange
        
        stats_frame = ttk.Frame(self.parent, padding="10")
        stats_frame.pack(fill=tk.X)
        self.stats_label = ttk.Label(stats_frame, text="Total Hosts Alive: 0 | Scanned: 0/0", font=("Helvetica", 10, "bold"))
    ...
    def ping_host(self, ip: str, timeout: int = 1) -> Tuple[str, bool]:
        ...
        try:
            ...
            output = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, 
                                  timeout=timeout + 1, startupinfo=startupinfo)
...
class PortScannerTab:
    ...
    def create_widgets(self):
        ...
        ttk.Label(header_frame, text="Port Scanner", font=("Helvetica", 16, "bold"), bootstyle="primary").pack()
        ttk.Label(header_frame, text="Scan specific TCP ports on a target host", font=("Helvetica", 9), bootstyle="secondary").pack()

        # Input Frame
        input_frame = ttk.Labelframe(self.parent, text="Target Configuration", padding="10", bootstyle="info")
        input_frame.pack(fill=tk.X, padx=10, pady=5)
        ...
        self.scan_btn = ttk.Button(btn_frame, text="Start Scan", command=self.start_scan, bootstyle=SUCCESS)
        self.scan_btn.pack(side=tk.LEFT, padx=5)
        
        self.stop_btn = ttk.Button(btn_frame, text="Stop", command=self.stop_scan, state=tk.DISABLED, bootstyle=DANGER)
        self.stop_btn.pack(side=tk.LEFT, padx=5)

        self.clear_btn = ttk.Button(btn_frame, text="Clear Logs", command=self.clear_logs, bootstyle="secondary-outline")
        ...
        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(prog_frame, variable=self.progress_var, maximum=100, bootstyle="striped-info")
        self.progress_bar.pack(fill=tk.X)
        self.status_lbl = ttk.Label(prog_frame, text="Ready", bootstyle="inverse-secondary")
        self.status_lbl.pack(pady=2, fill=tk.X)

        # Results
        res_frame = ttk.Labelframe(self.parent, text="Scan Output", padding="10", bootstyle="info")
        res_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

        self.console = ScrolledText(res_frame, width=80, height=15, font=("Consolas", 9), autohide=True)
        self.console.pack(fill=tk.BOTH, expand=True)
        
        # Tags for coloring
        self.console.tag_config("open", foreground="#5cb85c", font=("Consolas", 9, "bold"))
        self.console.tag_config("closed", foreground="gray")
        self.console.tag_config("error", foreground="#d9534f")
        self.console.tag_config("info", foreground="#5bc0de")
...
class IPGameTab:
    ...
    def setup_ui(self):
        ...
        # Header
        title = ttk.Label(container, text="Public vs Private IP Game", font=("Helvetica", 16, "bold"), bootstyle="primary")
        ...
        self.score_label = ttk.Label(score_frame, text="Current Streak: 0", font=("Helvetica", 12, "bold"), bootstyle="info")
        self.score_label.pack(side=tk.LEFT, padx=20)
        
        self.highscore_label = ttk.Label(score_frame, text="High Score: 0", font=("Helvetica", 12))
        self.highscore_label.pack(side=tk.RIGHT, padx=20)
        
        # The Question (IP Address)
        self.question_frame = ttk.Labelframe(container, text="Current IP", padding="20", bootstyle="primary")
        self.question_frame.pack(fill=tk.X, pady=20)
        
        self.ip_display = ttk.Label(self.question_frame, text="PRESS START", font=("Consolas", 24, "bold"), justify=tk.CENTER, bootstyle="inverse-primary")
        self.ip_display.pack(fill=tk.X)
        
        # Timer Bar
        self.timer_var = tk.DoubleVar(value=100)
        self.timer_bar = ttk.Progressbar(container, variable=self.timer_var, maximum=100, length=300, orient=tk.HORIZONTAL, bootstyle="danger")
        ...
        self.btn_private = ttk.Button(btn_frame, text="PRIVATE", command=lambda: self.check_answer("Private"), width=15, bootstyle="warning")
        self.btn_private.pack(side=tk.LEFT, padx=10)
        
        self.btn_public = ttk.Button(btn_frame, text="PUBLIC", command=lambda: self.check_answer("Public"), width=15, bootstyle="info")
        self.btn_public.pack(side=tk.LEFT, padx=10)
        
        # Control Buttons
        self.btn_start = ttk.Button(container, text="Start Game", command=self.start_game, width=20, bootstyle="success")
        self.btn_start.pack(pady=20)
        
        # Feedback label
        self.feedback_label = ttk.Label(container, text="", font=("Helvetica", 11, "bold"))
    ...
    def next_round(self):
        ...
        self.ip_display.config(text=self.current_ip, bootstyle="inverse-primary")
    ...
    def check_answer(self, user_guess):
        ...
        if user_guess == correct_answer:
            self.score += 1
            self.score_label.config(text=f"Current Streak: {self.score}")
            self.feedback_label.config(text="Correct!", bootstyle="success")
    ...
    def game_over(self, reason):
        ...
        self.ip_display.config(text="GAME OVER", bootstyle="inverse-danger")
        self.feedback_label.config(text=reason, bootstyle="danger")
...
class PasswordGeneratorTab:
    ...
    def setup_ui(self):
        ...
        # Options Frame
        opts_frame = ttk.Labelframe(container, text="Options", padding="15", bootstyle="info")
        ...
        # Checkboxes - Using Toggle Style
        check_frame = ttk.Frame(opts_frame)
        check_frame.pack(fill=tk.X, pady=10)
        ttk.Checkbutton(check_frame, text="A-Z (Uppercase)", variable=self.use_upper, bootstyle="round-toggle-success").pack(anchor=tk.W, pady=2)
        ttk.Checkbutton(check_frame, text="a-z (Lowercase)", variable=self.use_lower, bootstyle="round-toggle-success").pack(anchor=tk.W, pady=2)
        ttk.Checkbutton(check_frame, text="0-9 (Digits)", variable=self.use_digits, bootstyle="round-toggle-success").pack(anchor=tk.W, pady=2)
        ttk.Checkbutton(check_frame, text="!@# (Symbols)", variable=self.use_symbols, bootstyle="round-toggle-success").pack(anchor=tk.W, pady=2)

        # Action Buttons
        btn_frame = ttk.Frame(container)
        btn_frame.pack(pady=20)
        ttk.Button(btn_frame, text="Generate Password", command=self.generate, bootstyle=SUCCESS).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Copy to Clipboard", command=self.copy_clipboard, bootstyle=SECONDARY).pack(side=tk.LEFT, padx=5)

        # Result Display
        res_frame = ttk.Labelframe(container, text="Generated Password", padding="15", bootstyle="success")
        res_frame.pack(fill=tk.X, pady=10)
        
        self.entry_res = ttk.Entry(res_frame, textvariable=self.generated_password, font=('Consolas', 12), state='readonly')
...
class NetworkToolSuite:
    def __init__(self, root):
        self.root = root
        self.root.title("Network Tool Suite V1.3.1 (Dark Mode)")
        self.root.geometry("950x800")
        
        # Note: resizable is handled by the Window class automatically usually, 
        # but explicit call ensures it works
        self.root.resizable(True, True)
        
        # Styles allow you to customize spacing, though bootstrap handles most
        style = ttk.Style()
        ...
        self.status_bar = ttk.Label(self.root, text="Network Tool Suite Ready", relief=tk.SUNKEN, anchor=tk.W, bootstyle="inverse-light")
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)

if __name__ == "__main__":
    # Create the window with a specific theme. 
    # Good dark themes: 'superhero', 'darkly', 'cyborg'
    # Good light themes: 'cosmo', 'flatly', 'journal'
